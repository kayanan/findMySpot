...
export const findReservationsByUser = async (filters: mongoose.FilterQuery<ReservationModel>,page:number,limit:number) => {
  const count= await ReservationDTO.countDocuments(filters);
   const result= await ReservationDTO.find(filters).populate("parkingSlot parkingArea user createdBy vehicleType rating").sort({createdAt:-1}).skip((page-1)*limit).limit(limit);
   return {result,count}
};
...
export const findActiveReservations = async (filters: mongoose.FilterQuery<ReservationModel>,page:number,limit:number) => {
  const count= await ReservationDTO.countDocuments(filters);
  const result= await ReservationDTO.find(filters).populate("parkingSlot parkingArea user").sort({createdAt:-1}).skip((page-1)*limit).limit(limit);
  return {result,count}
};
